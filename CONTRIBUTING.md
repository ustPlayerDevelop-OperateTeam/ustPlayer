...
- [ ] **测试**：`uv run pytest` 全部通过（185 个用例，`QT_QPA_PLATFORM=offscreen` 已在 `tests/conftest.py` 顶层设置）。
...
> 项目已有自动化测试（见上，185 个用例），配合**类型检查**与**手动验证**共同保障；改动功能或修复 Bug 时请在 `tests/` 补充或调整对应测试。
