...
> [!IMPORTANT]
> **本决定已于 2.0 开发后期被推翻，实际采纳方案 B（播放器自绘）。**
> Spike 0a/0b 只验证了「渲染器能不能实时出帧」，**没有验证「渲染器画出来的画面
> 与 1.1.x 播放器一致」**——而这正是播放器最要紧的一点。实测（2026-09 首次真正
> 跑起播放器并抓帧对比）发现渲染器的版式与 1.1.x 播放器明显不同：
> 音名与歌字叠在同一位置、字号与留白都按「导出视频」的规则算，
> 缺少播放器应有的观感。渲染器是为**视频导出**设计的，播放器画面是播放器自己的职责。
>
> 因此 2.0 改为：**播放器自绘**（`Views/Rendering/PlayerCanvasRenderer.cs`，
> 按 `player.py` 的 `paintEvent` 逐条移植版式，含音名 / 音高曲线 / 歌字 /
> 左上信息 / BPM / 播放时间 / LRC / 版权），**渲染器只留给视频导出**。
> 会话不再依赖渲染器：`Timing/PlaybackSessionFactory` 负责组装，
> 于是「想播放就必须经过渲染器合成器」这条隐藏耦合被解除。
>
> 下面记录的是当时的判断与实测数据，保留以便对照；**不要再据此把播放器改回出帧方案**。
...
- ~~`player.py` 那 806 行的迁移量从「QPainter → DrawingContext 全量直译 + 字体系统重做」
  ...
  **实际未省下**：最终仍按 1.1.x 的绘制规则把版式移植到了
  `PlayerCanvasRenderer`（约 400 行，含音高曲线的夹紧规则）。
  省下的是**视频导出**的绘制实现（那部分确实只有一份）。
- ~~跨平台不再需要解决 Avalonia 侧的字体与文本排版（含 CJK 回退）；
  macOS/Linux 上的字体回退由渲染器负责。~~
  **自绘后这项好处不再成立**：文字排版回到 Avalonia 侧，CJK 回退依赖系统字体。
  （Avalonia 没有运行时字体注册 API，见 `docs/plan-deviations.md` D9。）
- ~~「预览 = 导出」由同一实现保证~~：自绘后**预览与导出是两套绘制实现**，
  观感需要在两者都改动时人工对齐——这是当初想避免的隐患，现已知情接受，
  因为「导出观感」与「播放器观感」本来就是两种需求。
- 4K 实时预览帧率不足 60 的取舍不再相关（Avalonia 自绘走 GPU 合成）。
