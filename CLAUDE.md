...
- 测试：`uv run pytest`（171 个用例，覆盖 `contracts` / `ustreader` / `settings` 七个子域 / `settings_store` / `settings_manager` / `i18n` / `player` / `uplr_io`）。跑单个用例：`uv run pytest tests/test_uplr_io.py::test_export_import_round_trip`。`QT_QPA_PLATFORM=offscreen` 在 `tests/conftest.py` 顶层设置，无显示器 / CI 也能跑 Qt 测试；新增测试放 `tests/`，约定见 `tests/conftest.py`。
...
- `core/contracts.py`：数据契约（`UstInfo` / `NoteInfo` / `PlayerLaunchParams`）+ 服务 Protocol（`UstParser` / `PlayerLauncher` / `ProjectIO`）+ 颜色 / 布尔工具 + `APP_VERSION`。
- `core/settings/`：每个分组一个信号驱动子域（`project` / `file` / `display` / `color` / `player` 含 `LyricSettings` / `theme`），各持属性 + `Signal` + `read_from` / `write_to` / `validate`；`SettingsManager` 只做组装与编排。
- `core/settings_store.py`：`Settings.json` 文件 I/O（分组→键值字典），首次运行自动迁移旧版 `Settings.ini`。
- `core/uplr_io.py`：`.uplr` 导入 / 导出。**新版 = ZIP 容器**（`Info.json` + 资源，导入解压到 `%LOCALAPPDATA%\ustPlayer\projects\<工程名>-<hash8>\`）；**旧文本格式仅可导入**（按 ZIP 魔数自动识别）。导入经 setter 写设置 → 触发信号 → UI 自动同步。
- `core/ustreader.py`：只解析 `.ust` 文本（**不支持** `.ustx`），默认 Shift-JIS，编码错误抛 `UnicodeDecodeError`。
- `core/player.py`：全屏 QPainter 播放器 + QtMultimedia 伴奏（无音频 / 失败时回退墙钟计时）。改动时保留文件顶部 `try/except` + `TYPE_CHECKING` 降级导入模式。
...
- **错误码**：用户可见错误用 `InfoBar.error("ERcodeXXX", "提示文案", ...)`，新错误码登记到 `ERcode.txt`（001–010、999 已占用）。
...
- ⚠️ 提交信息以 `pass` 开头会**跳过 CI 构建**（留给纯文档提交）；以 `v` 开头（且非 `pass`）会**触发自动发版**——除非确实要发版，否则**绝不要**用 `v` 前缀。
- CI 从 `ChangeLog.md` 的 `# v{版本}` 一级标题提取 Release 说明；顶部 `## Unreleased` 不被提取。用户可见变更补进 `## Unreleased` 的「更新内容」列表。
- 版本号用语义化（`pyproject.toml` 当前 `1.1.0b1` ↔ `contracts.APP_VERSION` "1.1.0 Beta 1"）；旧的日期式版本号（如 `v26f19`）已废弃，不要重新引入。
- `pyside6-fluent-widgets` 刻意**不带** `[full]` extra（那会引入 ~120MB scipy/numpy/pillow/colorthief 死依赖），不要"修复"这一点。

## 平台

仅 Windows：`ustplayer/ui/main_window.py` 使用 `winreg` 读取系统强调色，WSL / Linux 上无法运行。构建 / 发版只通过 GitHub Actions（windows-latest 上的 Nuitka standalone，另有 C++ `uplr_converter` 任务与发版打包）。
