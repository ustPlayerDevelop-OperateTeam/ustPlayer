...
> [!IMPORTANT]
> **ustPlayer 2.0 是把整个程序从 Python / PySide6 重写为 C# / .NET 10 + Avalonia 11 的迁移版本。**
...
- **全新的 Fluent 界面**：基础 / 文件 / 播放器 / 歌词 / 设置 五个页面，
  卡片式布局，支持三语（简体中文 / English / 文言），亮暗主题跟随系统。
- **伴奏播放**：播放器按伴奏的真实播放位置驱动时间轴；音频不可用时自动降级为墙钟计时，
  画面与歌词时序不跳变。
- **视频导出**：可选分辨率（含自定义）与帧率、可混入伴奏，导出期间可取消（并清理半成品）。
- **文件拖放**：`.uplr` / `.uprd` 拖到基础页即导入，`.ust` 拖到文件页、`.lrc` 拖到歌词页即选中。
- **自定义外观**：主题模式、强调色（跟随系统 / 自定义）、窗口效果（Mica / 亚克力）。
- **命令行参数**：`--play <UST 路径>` 跳过主界面直接全屏播放；
  `--music <伴奏路径>` 覆盖伴奏设置（便于不进设置界面直接连伴奏试听）；
  `--page <basic|file|player_style|lyric|settings>` 启动时直接打开指定页面。

## 🐛 修复

- **伴奏不播放**（两个叠加的缺陷，都会让「音频已就绪、时长正确」却完全没有声音）：
  1. 音频看门狗（等就绪 / 等播放开始 / 超限降级）**生产代码里从来没有人驱动过**——
     它只在单元测试里被调用。后果是音频一次都没开始播，时间轴停在起点，
     播放器看起来像没做出来。
  2. 后端在 `AudioBackendFactory` 里一创建就开始解析，小伴奏几十毫秒就绪；
     而会话要等渲染器初始化完（几百毫秒）才订阅 `Ready`——**事件早已发过**，
     会话永远收不到，于是既不播也不降级。现在看门狗会补上「该开始播了」这一步。
- **播放中途被误判为「已加载但没播」而掐掉**：`Play()` 是异步的，刚发出时
  `IsPlaying` 仍为 false；现在给一次宽限，并在**播放真正开始后**才停止看门狗，
  避免定时器带着过期状态回头把正常播放降级。
- **libvlc 位置读数异常导致时间轴瞬间跳到结尾**：位置超过媒体时长时按 0 处理并记日志
  （某些容器/编码组合下会读到离谱的值，表现为播放刚开始就结束）。

## 🧰 开发与验证

- `build/capture-window.ps1`：把主窗口指定页面截成 PNG。
  卡片间距、导航项位置这类**只能看渲染结果**的问题，靠它能量出像素来核对
  （无头测试构造不了窗口，见 `docs/adr-0002-window-chrome.md`）。
