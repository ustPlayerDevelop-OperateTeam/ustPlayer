...
- `core/video_exporter.py` + `core/renderer_ffi.py`：视频导出 —— ctypes 封装 uPlRender DLL（`ustplayer_renderer.dll`，从主程序 `renderer/` 子目录加载），渲染 MP4 并写 `.uprd` 工程文件。时序以「音频播完」为结束边界；依赖 `ffmpeg` / `ffprobe` 在 PATH。
