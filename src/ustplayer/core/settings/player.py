...
class PlayerSettings(QObject):
    ...
    def read_from(self, config):
        """从 [PlayerSettings] 与 [LyricSettings] 两个分组读取。"""
        if "PlayerSettings" in config:
            cs = config["PlayerSettings"]
            self._lyric_pos = cs.get("lyric_pos", self._lyric_pos)
            self._silent_display = cs.get("silent_display", self._silent_display)
            self._silent_custom_text = cs.get("silent_custom_text", self._silent_custom_text)
            self._end_display = cs.get("end_display", self._end_display)
            self._end_custom_text = cs.get("end_custom_text", self._end_custom_text)
            self._pitch_placeholder = cs.get("pitch_placeholder", self._pitch_placeholder)
