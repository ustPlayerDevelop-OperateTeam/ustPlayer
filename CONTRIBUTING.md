...
- **发版只由标签推送触发**：`git tag <版本> && git push origin <版本>`（标签名带不带 `v` 前缀均可，如 `1.1.0` / `v1.1.0`）。提交信息不参与发版判定——以 `v` 开头的提交信息**不会**触发发版，但仍请保持常规前缀让历史整洁。发版由维护者负责。
...
- 找不到与推送标签对应的小节时，发布会**直接中止**（不发布占位符）；发版前 CI 还会校验「标签推导版本 ↔ `contracts.APP_VERSION`」一致，不一致同样中止。
...
> 项目已有自动化测试（见上，220 个用例），配合**类型检查**与**手动验证**共同保障；改动功能或修复 Bug 时请在 `tests/` 补充或调整对应测试。
