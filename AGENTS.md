...
- 没有自动化测试，也没有已提交的 linter 配置；验证方式是手动的（启动应用、加载 .ust、播放）。类型检查目标是 **Pylance / pyright Standard 模式 0 error**（CONTRIBUTING.md 有说明，例如 `npx --yes pyright main.py src`；未提交 `pyrightconfig.json`——需要时在本地自行创建）。
...
- `src/ustplayer/core/`：
  ...
  - `settings/` —— 每个设置分组对应一个信号驱动的子域（原 ini 段，键保留在 `Settings.json` 中）：`project.py`（`ProjectSettings`）、`file.py`（`FileSettings`）、`display.py`（`DisplaySettings`）、`color.py`（`ColorSettings`）、`player.py`（`PlayerSettings`+`LyricSettings`）、`theme.py`（`ThemeSettings`，不导出到 uplr）。每个类自行持有属性 + `Signal`s + `read_from`/`write_to`/`validate`。
