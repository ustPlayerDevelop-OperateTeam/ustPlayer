...
internal static class PlayerLauncher
{
    ...
    /// <returns>
    /// 已开始加载的后端；未配伴奏、文件不存在或后端不可用时返回 <see langword="null"/>。
    /// </returns>
    /// <remarks>
    /// 选型为 **LibVLCSharp**（跨平台且可延伸到 Android），实现与降级都在
    /// <see cref="AudioBackendFactory"/> 里。返回 <see langword="null"/> 时
    /// <see cref="PlaybackSession"/> 立即按**墙钟计时**：画面与歌词时序完整可用，
    /// 只是没有伴奏声——这是刻意的降级，而不是「音频静默失效」。
    /// </remarks>
    internal static IAudioBackend? CreateAudioBackend(string? musicPath) =>
        AudioBackendFactory.Create(musicPath);
}
